...
if sys.version_info < (3, 9):
	sys.exit("1note2enex: This package requires Python 3.9+")

def main():

	import argparse

	parser = argparse.ArgumentParser(description='Convert Microsoft OneNote files to ENEX (Evernote Export) format.', allow_abbrev=False)
	parser.add_argument("onefile", metavar='<onefile>', help="Source '.one' or '.onetoc2' Microsoft OneNote file")
	parser.add_argument("--output", '-O', metavar='<enex-filename>',
						help="Filename of output ENEX file")
	...
	if options.output:
		print("Making ENEX file %s..." % (options.output,), file=sys.stderr, end='', flush=True)
		onenote.MakeEnexFile(options.output, options)
		print("done", file=sys.stderr)
