...
[1note2enex.py](#1note2enex) command line application generates an ENEX (Evernote Export) file from the provided OneNote section or notebook file, suitable for importing into Apple Notes or other note-taking applications.
...
```
python 1note2enex.py <OneNote filename> --output <output.enex> [common options]
```

Features:
- Converts OneNote pages to Evernote notes with proper ENML (Evernote Markup Language) formatting
- Preserves text formatting (bold, italic, underline, strikethrough)
- Maintains document structure (headers, paragraphs, lists, tables)
- Embeds images and file attachments as base64-encoded resources
- Preserves creation and modification timestamps
- Includes author information when available

The generated ENEX file can be imported into:
- **Apple Notes**: File → Import to Notes → Select the .enex file
- **Evernote**: File → Import Notes → Select the .enex file
- Other applications that support the ENEX format
